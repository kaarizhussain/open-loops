...
function parseDue(text, from) {
  ...
  /* Each month spelled out rather than `abbrev + [a-z]*`.
   *
   * That wildcard ate the rest of whatever word began with those three letters, and the
   * word boundary after it was then satisfied — so "maybe 3 of us can join" was May 3rd,
   * "we will decide 5 things" was December 5th, and "marketing 5 campaign numbers" was
   * March 5th. Six ordinary sentences out of eight produced a date.
   *
   * A wrong deadline is worse than no deadline. No date means the item ranks on age; a
   * fabricated one either makes something look overdue that is not, or parks a real
   * commitment months away where nobody will see it again. */
  var m = t.match(/\b(?:by|before|on)?\s*(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t|tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\.?\s+(\d{1,2})(?:st|nd|rd|th)?\b/);
  if (m) {
    var d = toUTC(from);
   ...
   * deadline seven days of false slack. */
  if (/\bthis week\b|\bend of week\b|\beow\b/.test(t)) {
    return toUTC(from).getUTCDay() === 5 ? iso(toUTC(from)) : nextDow(from, 5);
  }
  return null;
}
...
// A delivery word inside a denial, question or future plan is not completion.
function delivered(text) {
  return DELIVER.test(text) && !/\?/.test(text) &&
...
/* Whichever action word comes first, because that is the next action. Testing one list
 * before the other instead lets the topic outrank the verb: "I'll book the pricing
 * review" is the assistant booking something, and matching EXEC_WORK first calls it the
 * executive's — the exact case the note above says it is not. */
function nextAction(t) {
  var a = t.search(EXEC_WORK), b = t.search(EA_WORK);
...
/* Who the sentence makes commit: the reader, a person by name, or someone else.
 *
 * First person — "I'll", "we'll", "let me", or chat's dropped subject ("Will send it
 * Thursday", "Sure — will get you the numbers") — is the reader. A single capitalised word
 * straight before "will" is a name: "Sarah will send the contract", "Dana will review it".
 * Anything else there — "they", "the vendor", "our legal team" — is somebody who is neither. */
var NOT_A_NAME = /^(they|he|she|it|this|that|we|our|the|their|your|when|what|who|how|where|why|which)$/i;
function committer(what) {
  var t = String(what || '');
...
function assignOwner(type, what, subject, principals) {
  ...
  /* The reader is the assistant, or works alone — which is whenever principals is a list,
   * and the Slack runner always passes one. Then ownership is grammatical: whoever the
   * sentence makes commit owns the loop. "I'll draft the onboarding doc" is the reader's,
   * whatever verb it uses; "Dana will send the signed copy" is the executive's; "Sarah will
   * send the contract" is Sarah's, to be chased for. A question put to the reader is
   * theirs to answer.
   *
   * This used to fall through to the verb split below and default to the executive — a
   * rule written when the reader WAS the executive. On the first run where the reader
   * supported somebody, their own promises filled the executive's pile. */
  if (Array.isArray(principals)) {
    if (type === 'unanswered_ask') return 'you';
    if (committer(what) === 'self') return 'you';
    return principalNamed(what, principals) ? 'exec' : 'them';
  }
  /* No principals: the historical shape, one unnamed executive reading their own inbox,
   * so "I" is that executive and the split is by the next action. The sentence decides and
   * the subject only breaks a tie — concatenating them once let a channel name overrule
   * the promise inside it, sending everything in #pricing to the executive. */
  return nextAction(what) || nextAction(subject) || 'exec';
}

/* Which principal a commitment was made in the name of — "Dana will send the signed copy".
 *
 * This used to route by who was on the conversation, falling back to the first principal
 * listed. That guessed: an item in a channel both principals sat in went to whichever was
 * listed first. Now the executive's pile only holds promises made in a principal's name,
 * so the name is the routing, and there is nothing left to guess. */
function principalNamed(what, principals) {
  var c = committer(what);
  if (c === 'self' || c === 'other') return null;
  return (principals || []).filter(function (p) {
    return String(p.label || p.name || '').trim().split(/\s+/)[0].toLowerCase() === c.toLowerCase();
  })[0] || null;
}
...
/* Whether a later message actually answers a question. Stricter than closure on purpose.
 * A delivery names what it delivers, so one shared word is evidence there; a channel is
 * full of generic work vocabulary, and "review the Q4 headcount plan" and "walk through
 * the rollout plan" share "plan" without either answering the other. So an answer must
 * share a word that says what the question was about. A missed answer leaves a question
 * open that a reply of `k` clears; a false one makes a real question vanish unseen. */
var GENERIC = ['plan', 'review', 'call', 'meeting', 'sync', 'update', 'document', 'docs', 'doc',
  'time', 'times', 'date', 'dates', 'thing', 'question', 'answer', 'info', 'detail', 'work',
  'look', 'check', 'confirm', 'side', 'sides', 'stuff', 'note', 'notes', 'number', 'start',
  'point', 'chance', 'minute'];
function telling(text) {
  return topics(text).filter(function (w) {
    return !GENERIC.some(function (g) { return g === w || sameTopic(g, w); });
  });
}
function answers(question, reply) {
  var q = telling(question), r = telling(reply);
  return q.some(function (a) { return r.some(function (b) { return sameTopic(a, b); }); });
}
...
function askDue(q, m, ours) {
  var d = ours ? null : parseDue(q, m.date), from = null;
  sentences(m.body).forEach(function (x) {
    if (!d && x !== q && DEADLINE.test(x) && !FIRM.test(x) && !LETS.test(x)) {
      d = parseDue(x, m.date);
      // Where the date came from, so the digest can quote it: it is not in the question.
      if (d) from = { text: x, from: m.from, same: true };
    }
  });
  return { due: d, dueFrom: from };
}
...
function detectLoops(messages, events, opts) {
  ...
  var out = [], held = [];   // held: ids of questions too young to raise yet
  var threads = {};
  ...
  Object.keys(threads).forEach(function (tid) {
    ...
    /* A channel read is a stream, not a thread: many conversations interleaved, with no
     * reply structure between them. The whole-thread counterparty above is the first
     * outsider who ever spoke, which in a channel made every commitment in it theirs —
     * promises made before they had even joined included — and put their name on the chase
     * note and their tier on the ranking. In a stream, attribution comes per commitment,
     * from the strongest evidence available:
     *
     *   1. a real thread participant — that is the thread path, not this one
     *   2. a person addressed by name: "Lena — can you confirm…"
     *   3. someone from the other side who spoke earlier that day, about the same thing
     *   4. the one other member, when the conversation declares only one — a DM
     *   5. otherwise nobody — so no name and no tier, never a borrowed one. */
    var stream = msgs.length > 0 && msgs.every(function (m) { return m.stream; });
    /* Rule 4 is not an assumption about a channel; a conversation with one other member is
     * a conversation with them. Taken from the declared members only, and only while nobody
     * else from outside has spoken — who happened to speak is what rule 3 distrusts. */
    var sole = null;
    if (stream) {
      var others = [];
      [].concat(msgs[0].to || []).forEach(function (t) {
        t = String(t).toLowerCase();
        if (side(t) !== execSide && others.indexOf(t) < 0) others.push(t);
      });
      if (others.length === 1 && msgs.every(function (m) {
        return m.out || side(m.from) === execSide || m.from.toLowerCase() === others[0];
      })) sole = others[0];
    }
    var byName = {};
    msgs.forEach(function (m) {
      if (!stream || m.out || side(m.from) === execSide || !m.fromName) return;
      var first = String(m.fromName).trim().split(/\s+/)[0].toLowerCase();
      if (first.length > 1 && !byName[first]) byName[first] = m.from;
    });
    var attribute = function (i, s) {
      var v = String(s).match(/^\s*(?:(?:hi|hey|thanks|thank you)[\s,]+)?([A-Za-z]+)\s*[,—–:!-]/i);
      if (v && byName[v[1].toLowerCase()]) return byName[v[1].toLowerCase()];
      var d = day(msgs[i].date);
      for (var k = i - 1; k >= 0 && day(msgs[k].date) === d; k--) {
        var p = msgs[k];
        if (!p.out && side(p.from) !== execSide && answers(s, p.body)) return p.from;
      }
      return sole;
    };
    ...
    msgs.forEach(function (m, i) {
      ...
      sentences(m.body).forEach(function (s) {
        /* Words inside double quotes are somebody else's. `Marcus wrote: "I'll send it
         * Friday."` contains a first-person promise, and because the reader typed the
         * sentence it was filed as theirs — the same misattribution as relaying "Sarah
         * will send it", one layer of punctuation deeper. Only the cue tests read the
         * stripped copy; the item still quotes the sentence exactly as written. Single
         * quotes are left alone, since every "I'll" contains one. */
        var own = s.replace(/"[^"]*"|“[^”]*”/g, ' ');
        // Somebody backing away from a commitment has not made one.
        if (hedged(own)) return;
        ...
        if (firm && !FIRM.test(own)) return;
        if (FIRM.test(own)) firm = true;
        ...
        if (COMMIT.test(own)) promises.push({ at: i, m: m, s: s, closer: null });
      });
    });
    ...
    msgs.forEach(function (n, j) {
      ...
      var evidence = n.attach ? [n.body]
        : sentences(n.body).filter(delivered);
      ...
      evidence.forEach(function (d) {
        ...
        var open = promises.filter(function (p) {
          return !p.closer && p.m.from.toLowerCase() === n.from.toLowerCase() && p.at < j;
        });
    ...
    promises.forEach(function (pr) {
      ...
      var due = parseDue(s, m.date), dueFrom = null;
      ...
      /* Chat splits one sentence across two bubbles. "I'll send the board deck." then,
       * eight seconds later, "By Thursday." The deadline was never absent — it was in
       * the next message, and the promise arrived undated. That is not a small loss: an
       * item with no date ranks on age instead of urgency, so a real commitment with a
       * real deadline sinks below things that matter less.
       *
       * This borrows from the reader's OWN side, which the note above warns against, so
       * it is deliberately far narrower than the backward walk: the very next message
       * only, the same person only, minutes apart, and only when that message is short
       * enough to be the tail of a sentence rather than a new thought. A longer line
       * that merely contains a date — "Thursday I'm out of office" — is not a deadline
       * for anything and is left alone.
       *
       * ponytail: a fixed character cap does the fragment test, so "by Friday at the
       * latest" is inherited and anything wordier is not. Widen it against real chat,
       * not against invented examples. */
      if (!due) {
        var nxt = msgs[i + 1];
        var gap = nxt ? new Date(nxt.date + ':00Z') - new Date(m.date + ':00Z') : -1;
        if (nxt && nxt.out === m.out &&
            String(nxt.from).toLowerCase() === String(m.from).toLowerCase() &&
            gap >= 0 && gap <= 5 * 60000 &&
            String(nxt.body || '').trim().length <= 25) {
          due = parseDue(nxt.body, nxt.date);
          if (due) dueFrom = { text: nxt.body, from: nxt.from, same: true };
        }
      }
      ...
      if (!closer && m.out && MEETINGY.test(s) && SCHEDULEY.test(m.body)) {
        ...
        // In a stream the hold must be with whoever this promise is attributed to, not
        // with the first outsider who ever spoke in the channel.
        var toWhom = stream ? attribute(i, s) : counterparty;
        bookedBy = events.filter(function (e) {
          ...
          if (toWhom && side(toWhom) !== execSide &&
              e.attendees.indexOf(String(toWhom).toLowerCase()) > -1) return true;
      ...
      out.push({
        ...
        who: m.out ? (stream ? attribute(i, s) : (counterparty || (m.to || [])[0])) : m.from, byUs: m.out,
        what: shorten(s, 110), said: day(m.date), due: due, dueFrom: dueFrom,
        closedOn: closer ? day(closer.date) : bookedBy ? day(bookedBy.start) : null,
        // Who closed it and with which message: the digest quotes the evidence, and the
        // spot check must not offer that message back as one it found nothing in.
        closedByWho: closer ? (closer.out ? 'you' : closer.from) : null,
    ...
    if (stream) {
      /* In a stream each question stands alone. The whose-turn rule below treats a thread
       * as one conversation, which an email thread or a real Slack thread is — there the
       * other side's next message is the answer. A channel is many conversations at once,
       * and under that rule whoever posted last settled every question in it: Sam posting
       * about numbers "answered" a question put to Lena, and a channel tracked one question
       * at most. Now a question stays open until the other side says something about what
       * it asked. */
      msgs.forEach(function (m, i) {
        var q = askIn(m);
        if (!q) return;
        ...
        out.push(m.out
          ...
          : { type: 'unanswered_ask', threadId: tid, subject: m.subject, who: m.from, byUs: true,
              ...
              due: ad.due, dueFrom: ad.dueFrom, excerpt: m.body, msgId: m.id });
      });
      ...
      if (pending.length) {
        ...
        if (fAge >= 2 || fDue) {
          out.push({
            ...
            due: fd.due, dueFrom: fd.dueFrom, excerpt: first.body, msgId: first.id
          });
        }
      }
    } else if (lastOut > -1) {
      ...
      if (ours && (oAge >= 2 || oDue)) {
        out.push({
          ...
          due: od.due, dueFrom: od.dueFrom, excerpt: ours.body, msgId: ours.id
        });
      }
    }
  });
  ...
  out.forEach(function (l) {
    ...
    l.owner = assignOwner(l.type, l.what, l.subject, principals);
    /* A promise made in somebody else's name is chased with them, so the note can say
       "Chase Sarah" — and no tier is borrowed from whoever happened to be talking. */
    if (l.owner === 'them' && l.type === 'owed_by_us') {
      var named = committer(l.what);
      if (named !== 'self' && named !== 'other') l.who = named;
    }
    l.principal = l.owner === 'exec' ? principalNamed(l.what, principals) : null;
  ...
  return { open: open, closed: closed, today: today, held: held,
           dark: { no_followup: darkFollowup, quietSeries: quietSeries } };
}
...
/* FNV-1a, because the key must not carry the sentence.
 *
 * The key used to end with `l.what` verbatim, which quietly defeated the one privacy
 * control this has: `storeText: false` blanks the `who` and `what` columns and was
 * documented as dropping the words, while column 0 still read
 * "owed_by_us|#deals|2026-09-01|I will wire the Ellsworth settlement to Avista on
 * Monday." Somebody who turned that on to keep sentences off disk did not get what the
 * setting promised.
 *
 * A hash keeps everything the key is for — stable across runs, unique per item, enough
 * to tell two promises in one thread on one day apart — and none of what it should
 * never have held. The readable parts stay readable; the sentence lives in its own
 * column, where the retention flag can actually remove it. */
function fnv(s) {
  var h = 0x811c9dc5;
...
function loopKey(l) {
  ...
  return [l.type, l.threadId, l.said, fnv(String(l.what || ''))].join('|');
}

var OWNER = {
  them: { title: 'Chase them', note: 'Someone else owes this. Your move is the nudge.' },
  you:  { title: 'Yours to handle', note: 'Logistics you can close out yourself.' },
