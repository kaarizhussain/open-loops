...
/* DELIVER matches the bare word "signed", so this sentence looks like a delivery
   and a delivery-looking sentence used to be dropped before it could become a loop.
   It commits, though, and a commitment is future tense by construction — tense
   decides, not vocabulary. The Slack fixture above only escaped this because
   "countersigned" has no word boundary before "signed". */
var trap = parseChannel([
  '=== Message from Lena Borg <lena@vectorfreight.com> (U0BU111AAA1) at 2026-08-01 10:00:00 UTC ===',
  'Message TS: ' + epoch(2026, 8, 1, 10),
  "We'll have the signed order form back to you Friday Aug 8."
].join('\n'), { channel: '#vf', members: [], tzOffset: 0 });
var caught = detectLoops(trap, [], { exec: 'kaarizh@gmail.com', today: '2026-08-12' }).open;
...
/* The other half of the rule: with nothing committed, delivery language still
   reads as delivery and opens nothing. */
var plain = [{ id: 'd1', threadId: 'td', subject: '#vf', from: 'lena@vectorfreight.com',
  to: ['kaarizh@gmail.com'], date: '2026-08-01T10:00', attach: false,
  body: 'Signed order form attached.' }];
assert.strictEqual(detectLoops(plain, [], { exec: 'kaarizh@gmail.com', today: '2026-08-12' }).open.length, 0,
